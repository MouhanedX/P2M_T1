...
function initMap() {
  ...
  routes.forEach((route) => {
    ...
    // Add hover listener to show/hide label
    const mainLine = routePolysList[2]; // Main polyline
    mainLine.on('mouseover', () => label.addTo(routeLayer));
    mainLine.on('mouseout', () => label.removeFrom(routeLayer));
    ...
    L.circleMarker([route.lat, route.lng], {
      ...
      .bindPopup(
        `<p class="popup-title" style="color: ${source.color}">${route.id}</p><p class="popup-sub">${route.rtuId} → ${route.to}</p><p class="popup-sub">Distance: ${route.distanceKm} km | Inland fiber route</p>`
      )
      .addTo(routeLayer);
  });

  // Draw backbone links
  backboneLinks.forEach((backbone) => {
    const source = rtus.find((rtu) => rtu.id === backbone.fromRtuId);
    if (!source) return;

    const anchors = [[source.lat, source.lng], ...(backbone.via || []), [central.lat, central.lng]];
    const path = chaikinSmooth(anchors, 2);
    const backbonePolysList = [];

    // Deep shadow for backbone
    backbonePolysList.push(L.polyline(path, {
      color: central.color,
      weight: 10,
      opacity: 0.1,
      lineCap: "round",
    ...
    const label = L.marker(midpoint, {
      ...
      icon: L.divIcon({
        className: "distance-label",
        html: `<span class="backbone-chip">${backbone.distanceKm} km</span>`,
        iconSize: [86, 28],
        iconAnchor: [43, 14]
      })
    });
    
    backbonePolylines[backbone.id] = backbonePolysList;
    backbonePolylines[backbone.id + "_label"] = label;
    
    // Add hover listener to show/hide label
    const mainBackbone = backbonePolysList[0]; // Main backbone polyline
    mainBackbone.on('mouseover', () => label.addTo(routeLayer));
    mainBackbone.on('mouseout', () => label.removeFrom(routeLayer));
  });

  // Central NOC marker
  const centralMarker = L.marker([central.lat, central.lng], {
    icon: L.divIcon({
      className: "draggable-noc",
      html: `<div class="noc-marker" style="background:${central.color}"></div>`,
      iconSize: [32, 32],
      iconAnchor: [16, 16]
    })
  });
  ...
  centralMarker
    .bindPopup(`<p class="popup-title">${central.id}</p><p class="popup-sub">${central.name} | ${central.city}, Tunisia</p><p class="popup-sub">Central Hub - 1x RTU + 5x Backbone connections</p>`)
    .addTo(rtuLayer);
  ...
  L.tooltip({
    ...
    .setLatLng([central.lat, central.lng])
    .setContent(`<strong>${central.id}</strong> NOC`)
    .addTo(rtuLayer);
  ...
  rtus.forEach((rtu) => {
    ...
    marker
      .bindPopup(`<p class="popup-title">${rtu.id}</p><p class="popup-sub">${rtu.name} | ${rtu.city}, Tunisia</p>`)
      .addTo(rtuLayer);
...
function updateKpisAndLists() {
  ...
  document.getElementById("kpi-rtus").textContent = `${rtus.length} RTUs + 1 NOC`;
  document.getElementById("kpi-routes").textContent = `${routes.length} Routes + ${backboneLinks.length} Backbone`;
  document.getElementById("kpi-avg").textContent = `Avg distance ${avg} km`;
}
